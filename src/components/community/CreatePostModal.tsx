...
export function CreatePostModal({ onClose, onSubmit, editingPost, submitting, uploadProgress }: CreatePostModalProps) {
  ...
  const inputStyle: React.CSSProperties = {
    width: '100%', background: '#1f2126', border: '1px solid #2e323a', borderRadius: 14,
    padding: '0.8rem', color: '#e4e4e7', fontFamily: 'inherit', fontSize: '1rem',
    marginBottom: '1.2rem', outline: 'none', boxSizing: 'border-box', resize: 'vertical',
  };
  const labelStyle: React.CSSProperties = {
    display: 'block', marginBottom: '0.4rem', fontWeight: 500, color: '#b0b5c0', fontSize: '0.9rem',
  };
    ...
    <Modal onClose={onClose}>
      ...
      <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
        ...
        <button
          ...
          onClick={() => onSubmit(text, category, story, imageFile)}
          disabled={submitting}
        >{submitting ? 'Posting...' : (editingPost ? 'Save \u270F\uFE0F' : 'Post \uD83C\uDF3F')}</button>
      </div>
    </Modal>
  );
}
