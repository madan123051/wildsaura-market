...
export function PostCard({ post, currentUser, onLike, onComment, onEdit, onDelete, onLoginClick }: PostCardProps) {
  ...
  const handleLike = () => { if (!currentUser) { onLoginClick(); return; } onLike(post.id, post.likes || []); };
  const handleComment = () => { if (!currentUser) { onLoginClick(); return; } if (!commentText.trim()) return; onComment(post.id, commentText); setCommentText(''); };

  return (
    <div style={{ background: '#16181c', borderRadius: 18, padding: '1.2rem', border: '1px solid #262a31', position: 'relative' }}>
      {isOwner && (
        <>
          <button style={{ position: 'absolute', top: '1rem', right: '1rem', background: 'none', border: 'none', color: '#8a8f98', fontSize: '1.2rem', cursor: 'pointer', padding: '0.2rem 0.4rem', borderRadius: 8 }}
          ...
          {showMenu && (
            <div style={{ position: 'absolute', top: '2.5rem', right: '1rem', background: '#1f2126', border: '1px solid #2e323a', borderRadius: 12, padding: '0.3rem 0', zIndex: 50, minWidth: 120, boxShadow: '0 4px 20px rgba(0,0,0,0.5)' }} onClick={(e) => e.stopPropagation()}>
              <button style={{ display: 'block', width: '100%', background: 'none', border: 'none', color: '#e4e4e7', padding: '0.6rem 1rem', fontSize: '0.85rem', cursor: 'pointer', textAlign: 'left' }} onClick={() => { setShowMenu(false); onEdit(post); }}>\u270F\uFE0F Edit Post</button>
              <button style={{ display: 'block', width: '100%', background: 'none', border: 'none', color: '#e76f51', padding: '0.6rem 1rem', fontSize: '0.85rem', cursor: 'pointer', textAlign: 'left' }} onClick={() => { setShowMenu(false); onDelete(post.id); }}>\uD83D\uDDD1\uFE0F Delete Post</button>
            </div>
          )}
        </>
      )}
      ...
      {showComments && (
        <div style={{ marginTop: '1rem' }}>
          <ul style={{ listStyle: 'none', padding: 0, maxHeight: 200, overflowY: 'auto', marginBottom: '0.8rem' }}>
            {(post.comments || []).length === 0 && <li style={{ padding: '0.5rem 0', color: '#555', fontSize: '0.9rem' }}>No comments yet.</li>}
            {(post.comments || []).map((c: CommentItem, i: number) => (
              <li key={i} style={{ padding: '0.5rem 0', borderBottom: '1px solid #23262e', fontSize: '0.9rem', color: '#d1d5db', display: 'flex', alignItems: 'flex-start', gap: '0.5rem' }}>
                ...
                <div><span style={{ fontWeight: 600, color: '#d4a373', marginRight: 6 }}>{c.username}</span>{c.text}</div>
              </li>
            ))}
          </ul>
          {currentUser ? (
