  ...
  return (
    <Modal onClose={onClose} maxWidth={420}>
      ...
      <div style={{ maxHeight: '50vh', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        {members.length === 0 ? (
          <div style={{ textAlign: 'center', color: '#6b7280', padding: '2rem 0' }}>No members yet. Be the first to join! 🌿</div>
        ) : (
          members.map((m) => (
            <div key={m.userId} style={{ display: 'flex', alignItems: 'center', gap: '0.7rem', background: '#1f2126', borderRadius: 12, padding: '0.6rem 0.8rem' }}>
              ...
              <div>
                ...
                <div style={{ color: '#8a8f98', fontSize: '0.75rem' }}>{m.joinedAt ? `Joined ${timeAgo(m.joinedAt)}` : 'Member'}</div>
              </div>
            </div>
          ))
        )}
