  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-brand-dark to-brand-primary/80 px-4">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-8">
        {/* Header */}
        <Link href="/" className="block text-center font-heading text-2xl font-bold text-brand-primary mb-6">
          🌿 WildSaura
        </Link>

        <div className="flex rounded-xl overflow-hidden border border-surface-border mb-8">
          {(["login", "signup"] as Mode[]).map((m) => (
            <button
              key={m}
              onClick={() => setMode(m)}
              className={`flex-1 py-2.5 text-sm font-medium capitalize transition-colors ${
                mode === m ? "bg-brand-primary text-white" : "text-gray-500 hover:bg-surface-muted"
              }`}
            >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {mode === "signup" && (
            <Input label="Full Name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Hari Prasad Sharma" leftIcon={<User size={16} />} required />
          )}
          <Input label="Email Address" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="hari@example.com" leftIcon={<Mail size={16} />} required />
